...
C variables to hold mulch C and N totals
     ...
     &    All_C, CHumusMean,CLitterMean, CManureMean
      ...
      If (lInput.eq.1) then
        ...
        write(91,8) 'time,','Date,','Min_N,','Humus_N,',
     !     'Humus_C,','Manure_N,','Manure_C,','Litter_N,',
     !     'Litter_C,','CO2_C,','Ammon_N,','All_N,','All_C,',
     !     'water,','Denitr,','CFlux,',
...
8     Format (1x,A5,T20,A5,T40,A6,T60,A8,
     &  	  T80,A8,T104,A9,T124,A9,T144,A9,
     &        T164,A9,T186,A6,T209,A8,T229,A6,T249,A6, 
     &        T271,A6,T293,A7,T313,A6, 
     &        T333,A11,T353,A11,T376,A11, 
     &        T396,A10,
     &        T418,A10,T438,A9,T458,A8)	 
...
csun Calculate the co2 the units are ug
	   do i=1,NumNP
C gsink_OM unit is ug CO2 cm-3 air, need to calculate all C_RespirationOM (ugC/domain)		   
...
C Fact now works while there is a plant 
     ...
     !       litterC*fact, C_RespirationOM*fact,Ammon_N*fact,
...
10    Format (1F12.4,',', A12,',', 20(F20.4, ','),F20.4)
      return
      end
